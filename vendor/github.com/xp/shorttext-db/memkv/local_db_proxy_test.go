...
func TestLocalDBProxy_Scan(t *testing.T) {
	...
	//	return nil
	//})
	r := local.Scan(Key("A1"), Key("A6"), 50, 100, false, nil)
	for _, v := range r {
		fmt.Println(*v)
	}
	//fmt.Println(r)
...
func TestLocalDBProxy_GetByRawKey(t *testing.T) {
	local := NewLocalDBProxy(1)
	var item *DBItem
	item = &DBItem{RawKey: []byte{1}, Val: []byte{1}, StartTS: 1, CommitTS: 1}
	local.Put(item)
	item = &DBItem{RawKey: []byte{1, 2}, Val: []byte{1, 2}, StartTS: 1, CommitTS: 2}
	local.Put(item)
	...
	//	return nil
	//})
	r, ok := local.GetByRawKey([]byte{1}, 1)
	if ok {
		fmt.Println(r.RawKey)
	}
	//fmt.Println(r)
	//result,_ := local.GetByRawKey(Key("B2"),50)
	//fmt.Println(*result)
