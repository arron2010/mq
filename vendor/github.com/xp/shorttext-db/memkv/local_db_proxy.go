...
func scan(db MemDB, startKey Key, endKey Key, ts uint64, limit int, desc bool, validate ValidateFunc) []*DBItem {
	result := make([]*DBItem, 0, 0)
	if !desc {
		db.AscendRange("IndexRawKey",
...
func (l *LocalDBProxy) Scan(startKey Key, endKey Key, ts uint64, limit int, desc bool, validate ValidateFunc) []*DBItem {
	result := scan(l.db, startKey, endKey, ts, limit, desc, validate)
