...
func CreatDB(id uint32, persist bool) (*DB, error) {
	db := &DB{}
	db.persist = persist
	db.id = id
	...
	var path string
	path = "/opt/data/" + "memdb_" + strconv.Itoa(int(id))
	var err error
	if db.persist {
		db.writer, err = newDbWriter(path)
		db.writer.sync = true
	}
	return db, err
}
...
func (db *DB) SetPersist(persist bool) {
	db.persist = persist
}
...
func (db *DB) LoadDB() error {
	...
	defer db.mu.Unlock()
	var err error
	db.writer.ForEach(func(item *DBItem) {
		tx := &Tx{
			db: db, writable: true,
		}
		_, _, err = tx.Set(item)
