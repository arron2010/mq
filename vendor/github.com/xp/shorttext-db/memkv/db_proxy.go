...
const prefixSchema = 109

func NewDBProxy() (KVClient, error) {
	instance := &DBProxy{}
	instance.remote = GetRemoteDBProxy() //NewRemoteDBProxy(server.GetNodeProxy(), collaborator.GetCollaborator())
...
func (d *DBProxy) choose(key []byte, ts uint64) KVClient {
	//return d.buffer
	if len(key) == 0 || key[0] == prefixSchema || ts == lockVer {
		return d.buffer
	} else {
		return d.remote
	}
