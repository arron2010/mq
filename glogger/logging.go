...
type Config struct {
	...
	// logstyle.ParseFormat for details on the supported verbs.
	//
...
type Logging struct {
	*ModuleLevels
	...
	multiFormatter *logstyle.MultiFormatter
...
func New(c Config) (*Logging, error) {
	...
	s := &Logging{
		...
		multiFormatter: logstyle.NewMultiFormatter(),
	}
...
func (s *Logging) SetFormat(format string) error {
	...
	formatters, err := logstyle.ParseFormat(format)
	if err != nil {
		return err
	}
...
func (s *Logging) ZapLogger(module string) *zap.Logger {
	...
	core := &Core{
		...
		Encoders: map[Encoding]zapcore.Encoder{
			...
			CONSOLE: logstyle.NewFormatEncoder(s.multiFormatter),
		},
		Selector: s,
		Output:   w,
	}
...
// Logger instantiates a new GLogger for the specified module. The module
// becomes the name of the logger and is used to determine which log levels are
// enabled.
func (s *Logging) Logger(module string) *GLogger {
