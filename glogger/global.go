...
const (
	//%{module}.%{shortfunc}
	defaultFormat = "※%{level:.4s}※ %{message} %{time:2006-01-02 15:04:05} %{shortfile}"
...
func init() {
	...
	LogPath = os.Getenv("GOPLATFORM_LOGPATH")
	if LogPath == "" {
		LogPath = "/opt/gologger.txt"
	}
	Global = logging
...
func MustGetLogger(module string) *GLogger {
