...
var (
	...
	MaxIdleTimeoutWithoutPing = 60 * time.Second
...
func NewPoolEx(
	...
	password string,
	dbName string,
	afterConnect func(conn *Conn) (*Conn, error),
) *Pool {
	if minAlive > maxAlive {
		minAlive = maxAlive
	}
	...
	pool := &Pool{
		...
		connect: func() (conn *Conn, err error) {
			conn, err = Connect(addr, user, password, dbName)
			if err != nil {
				return nil, err
			}
			if afterConnect != nil {
				conn, err = afterConnect(conn)
				if err != nil {
					return nil, err
				}
			}
...
func (pool *Pool) putConnection(connection Connection) {
	...
	case pool.readyConnection <- connection:
		fmt.Println("putConnection--->pool.readyConnection <- connection-->242")
		return
	default:
	}
...
func (pool *Pool) spawnConnectionsIfNeeded() bool {
	...
	pool.synchro.Unlock()
	//fmt.Println("startNewConnections-->pool.minAlive-->",pool.minAlive," totalCount-->",totalCount)
	if needSpanNew <= 0 {
		return false
	}
	//fmt.Println("startNewConnections-->pool.minAlive-->",pool.minAlive," totalCount-->",totalCount)
...
func (pool *Pool) ping(conn *Conn) error {
	...
	err := conn.Ping()
	if err != nil {
		pool.logFunc(`Pool: ping query fail: %s [Connection Id:%d]`, err.Error(), conn.GetConnectionID())
	}
